...
public class OldMessage
{
    ...
    private OldHeader _header { get; } = new OldHeader();
    ...
    public OldMessage(OldHeader header, string payload)
    {
    ...
    /// [N bytes] - Header <see cref="OldHeader"/>
    ...
    /// <returns></returns>
    public static bool Deserialize(byte[] data, out OldMessage? message)
    {
        try
        {
            message = null;
            ...
            if (!OldHeader.Deserialize(headerBytes, out var header) || header == null)
            {
                return false;
            }
            ...
            message = new OldMessage(header, payload);
            return true;
        }
